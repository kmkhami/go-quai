...
type unconfirmedBlocks struct {
	slice  chainRetriever // Blockchain to verify canonical status through
...
func newUnconfirmedBlocks(slice chainRetriever, depth uint) *unconfirmedBlocks {
...
func (set *unconfirmedBlocks) Shift(height uint64) {
	...
	for set.blocks != nil {
		...
		header := set.slice.GetHeaderByNumber(next.index)
		switch {
		case header == nil:
		...
		default:
			...
			for number := next.index; !included && number < next.index+uint64(set.depth) && number <= height; number++ {
				if block := set.slice.GetBlockByNumber(number); block != nil {
