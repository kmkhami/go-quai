...
var (
	...
	EBlockCacheFlag = cli.IntFlag{
		Name:  "cache.eblock",
		Usage: "Percentage of cache memory allowance to use for external block caching (default = 5% full mode, 20% archive mode)",
		Value: 5,
	}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	case ctx.GlobalBool(DeveloperFlag.Name):
		...
		if cfg.Miner.Etherbase != (common.Address{}) {
			...
			developer, err = ks.NewAccount(passphrase, cfg.Genesis.Config.ChainIDRange())
			if err != nil {
...
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Ethereum) {
	...
	if err != nil {
		Fatalf("Failed to register the Quai Network service: %v", err)
	}
	if cfg.LightServ > 0 {
...
// RegisterEthStatsService configures the Quai Network Stats daemon and adds it to
// the given node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Quai Network Stats service: %v", err)
	}
}
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	types.QuaiNetworkContext = genesis.Config.Context
	return genesis
}
...
func MakeChain(ctx *cli.Context, stack *node.Node) (*core.Core, ethdb.Database) {
	...
	protocol, err := core.NewCore(chainDb, nil, nil, nil, config, ctx.GlobalString(DomUrl.Name), makeSubUrls(ctx), engine, cache, vmcfg, &core.Genesis{}, nil)
	if err != nil {
