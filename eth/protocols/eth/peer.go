...
func (p *Peer) RequestHeadersByHash(origin common.Hash, amount int, dom bool, reverse bool) error {
	p.Log().Debug("Fetching batch of headers", "count", amount, "dom", dom, "fromhash", origin, "reverse", reverse)
	query := GetBlockHeadersPacket{
...
func (p *Peer) RequestHeadersByNumber(origin uint64, amount int, dom bool, reverse bool) error {
	p.Log().Debug("Fetching batch of headers", "count", amount, "fromnum", origin, "dom", dom, "reverse", reverse)
	query := GetBlockHeadersPacket{
