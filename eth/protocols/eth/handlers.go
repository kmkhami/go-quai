...
func answerGetBlockHeadersQuery(backend Backend, query *GetBlockHeadersPacket, peer *Peer) []*types.Header {
	...
	fmt.Println("query: ", query.Origin.Number, query.Origin.Hash)
	...
	for !unknown && len(headers) < int(query.Amount) && bytes < softResponseLimit &&
		...
		case hashMode && query.Reverse:
			query.Origin.Hash, query.Origin.Number = backend.Core().GetAncestor(query.Origin.Hash, query.Origin.Number, uint64(1), &maxNonCanonical)
	...
	for _, header := range headers {
		fmt.Println(header.NumberArray())
	}
