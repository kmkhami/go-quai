...
func New(checkpoint uint64, stateDb ethdb.Database, mux *event.TypeMux, core Core, dropPeer peerDropFn) *Downloader {
	dl := &Downloader{
		stateDB:       stateDb,
		mux:           mux,
		checkpoint:    checkpoint,
		queue:         newQueue(blockCacheMaxItems, blockCacheInitialItems, core),
		peers:         newPeerSet(),
		core:          core,
		dropPeer:      dropPeer,
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, number uint64) (err error) {
	...
	fetchers := []func() error{
		//TODO: modify other functions to go backwards
...
func (d *Downloader) fetchHead(p *peerConnection) (head *types.Header, err error) {
	...
	fetch := 1
	go p.peer.RequestHeadersByHash(latest, fetch, false, true)
...
func (d *Downloader) fetchHeaders(p *peerConnection, from uint64) error {
	...
	getHeaders := func(from uint64) {
		...
		if skeleton {
			...
			if common.NodeLocation.Context() == common.PRIME_CTX {
				skeleton = false
				go p.peer.RequestHeadersByNumber(from, MaxSkeletonSize, false, true)
			} else {
				go p.peer.RequestHeadersByNumber(from, MaxSkeletonSize, true, true)
			}

		} else {
			p.log.Trace("Fetching full headers", "count", MaxHeaderFetch, "from", from)
			go p.peer.RequestHeadersByNumber(from, MaxHeaderFetch, false, true)
		}
	}
	...
	for {
		...
		case packet := <-d.headerCh:
			...
			var commonAncestor bool
			headers := packet.(*headerPack).headers
			if skeleton {
				fmt.Println("headers", headers)
				//TODO sizing
				copy(skeletonHeaders, headers)
				if len(skeletonHeaders) > 0 {
					// might have to check the n-1 block
					commonAncestor = d.core.HasBlock(skeletonHeaders[len(skeletonHeaders)-1].Hash(), skeletonHeaders[len(skeletonHeaders)-1].NumberU64())
				}
			} else {
				commonAncestor = true
			}
			...
			if commonAncestor && packet.Items() == 0 {
